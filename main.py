...
def get_class(model_path="Путь к модели", labels_path="путь к меткам", image_path="Путь к картинке"):    
    ...
    image = Image.open("file_0.jpg").convert("RGB")
...
def photo_ai(message):
    ...
    bot.reply_to(message, f"Картинка успешно загружена, идёт классификация")    
        
    try:
        class_name, confidence = get_class(image_path)
        bot.send_message(message.chat.id, f"Объект на изображении: {class_name}\nУверенность: {confidence:.2%}")
    except Exception as e:
        bot.send_message(message.chat.id, f"Ошибка при классификации: {e}")
# Запускаем бота
bot.polling()
